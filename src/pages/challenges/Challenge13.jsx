...
const BACKSTORY = [
  ...
  "The internal diagnostics dashboard still logs deploy metadata — including, inadvertently, the exact timestamp of when the last archive was created.",
  "Your mission: find the archive. Check the /info endpoint. The filename pattern follows a common naming convention you've seen in other CI pipelines.",
]
...
export default function Challenge13({ apiBase }) {
  ...
  return (
    <ChallengeShell id={13} title="Forgotten Backup" vuln="Sensitive Data Exposure" difficulty="easy" backstory={BACKSTORY}>
      <div style={panel}>
        <div style={header}>
          <span style={{ color: '#38bdf8', fontSize: '0.78rem', letterSpacing: '0.1em', fontWeight: 700 }}>NEOCORP WEBROOT PROBE</span>
          <span style={{ color: '#475569', fontSize: '0.68rem' }}>internal dashboard</span>
        </div>

        <div style={{ padding: '22px' }}>
          <div style={{ color: '#475569', fontSize: '0.72rem', marginBottom: 6 }}>
            Start your recon here:
          </div>
          <button onClick={() => fetchUrl(`${baseUrl}/info`)} disabled={loading} style={pathBtn}>
            GET {baseUrl}/info
          </button>

          <div style={{ marginTop: 20 }}>
            <label style={lbl}>TRY A PATH</label>
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                ...
                autoComplete="off"
              />
              <button onClick={() => fetchUrl(custom)} disabled={loading || !custom} style={btn}>Fetch</button>
            </div>
          </div>
        </div>
      </div>
...
const pathBtn = { display: 'block', width: '100%', textAlign: 'left', padding: '8px 12px', background: 'rgba(59,130,246,0.06)', border: '1px solid rgba(59,130,246,0.15)', color: '#e2e8f0', borderRadius: 6, cursor: 'pointer', fontFamily: "'JetBrains Mono',monospace", fontSize: '0.78rem' }
